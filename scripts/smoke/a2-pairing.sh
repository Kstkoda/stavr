...
# separate $COWIRE_HOME directories ("nas" and "device"), then revokes the
# device. Verifies:
#   1. `cowire pair bootstrap` returns a 6-digit code.
#   2. `cowire pair remote-host` exchanges it for a token + persists the
#      pairing on the device side at $COWIRE_HOME/devices.json mode 0600.
#   3. `cowire devices list` shows the new device.
#   4. `cowire devices revoke <id>` removes it from active-only listing.
#
...
TMP="$(mktemp -d 2>/dev/null || mktemp -d -t cowire-a2-smoke)"
...
COWIRE_HOME="$NAS_HOME" $CLI daemon start --port "$PORT" --db "$NAS_DB" --log-format json &
DAEMON_PID=$!
...
CODE=$(COWIRE_HOME="$NAS_HOME" $CLI pair bootstrap --daemon-url "http://127.0.0.1:$PORT" | python -c 'import json,sys; print(json.load(sys.stdin)["code"])' 2>/dev/null \
  || COWIRE_HOME="$NAS_HOME" $CLI pair bootstrap --daemon-url "http://127.0.0.1:$PORT" | grep -oE '"code": *"[0-9]{6}"' | grep -oE '[0-9]{6}')
if [[ ! "$CODE" =~ ^[0-9]{6}$ ]]; then
...
COWIRE_HOME="$DEV_HOME" $CLI pair remote-host \
...
LIST_OUT=$(COWIRE_HOME="$NAS_HOME" $CLI devices list --db "$NAS_DB")
...
COWIRE_HOME="$NAS_HOME" $CLI devices revoke "$DEVICE_ID" --db "$NAS_DB" >/dev/null \
  || { echo "FAIL: devices revoke failed"; exit 1; }
LIST_AFTER=$(COWIRE_HOME="$NAS_HOME" $CLI devices list --db "$NAS_DB")
