...
The following operations are **NEVER tier**. They are out of scope for Cowire's GitHub adapter even with human approval. The user runs them manually with full awareness of what they're doing:
...
- **Bounded blast radius for the agent.** The worst Cowire can do with `gh` is "open a noisy PR" or "make a wrong comment." Recoverable in minutes by a human. No history rewrites, no silent permission changes.
