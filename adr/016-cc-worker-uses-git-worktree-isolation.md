...
A single Stavr daemon will commonly run two or more Claude Code workers in the *same* repository — one fixing B-001 on `feat/b001`, another addressing review comments on `feat/b002`, a third spiking on `experiment/x`. The naive design (each CC worker sets `cwd` to the shared repo root and uses `git checkout <branch>`) corrupts immediately:
...
Every `cc`-type worker spawns into a dedicated `git worktree`. The default path is `<repo_path>/.stavr-worktrees/<worker-name>`; the `worktree_base` spawn parameter can override (e.g. `~/.stavr/worktrees/<repo-hash>/<worker-name>` for users who want them outside the repo).
...
4. Write `<worktree-path>/.stavr-mcp.json` so the spawned CC connects to the running daemon.
...
- **Worktree paths under `.stavr-worktrees/` should be `.gitignore`-ed in the host repo.** We do not automatically modify the user's `.gitignore`; the contributing guide documents the recommendation. Future improvement: write `.gitignore` inside `.stavr-worktrees/` so the host repo never sees them as untracked anyway.
