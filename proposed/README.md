# cowire — proposed changes

Concrete artifacts for the cowire build. Drop into the repo, review, land incrementally. Nothing here touches existing code paths until you wire it up.

## What cowire is

**cowire is a local-first orchestration layer that plans multi-step work, routes each step to the right AI model under a cost budget, and lets you plug in anything as a connector — with a visual toolkit your non-technical users can actually use.**
...
2. **Connector bus.** A standardized way to wrap any external service or local capability into cowire. MCP servers, REST APIs, OAuth-protected SaaS, LAN-only controllers (Unifi), home automation (Wiser), game platforms (Roblox, Unity), webhooks, cron schedules, SMTP — all the same orange-brick shape with a uniform `Connector` interface. Anyone can add a new one.
...
4. **Visual toolkit.** A DUPLO-style canvas where the cowire bus runs across the middle, external services live above (cloud, needs auth), local capabilities live below (your machine), and brain bricks plug into a socket at the end of the bus. Click any brick to configure it — each brick owns its own form fields. Drag from "The Shelf" to add new pieces.

Underneath all of this, cowire has a trust-scope and audit-log mechanism that makes "approve a plan, walk away, come back to a result" actually safe. **That mechanism is plumbing, not the headline.** See [`POSITIONING.md`](./POSITIONING.md) for the one-pager on what cowire is and is not.

## Prior art note

The trust-scope-plus-audit primitive that cowire uses to make plans safely executable is structurally the same idea as Tessera Protocol (`tessera-protocol.github.io/tessera`), which shipped first. Cowire's authority layer is independently derived but not novel. Tessera is narrowly an authority guard pattern; cowire is a broader orchestration system that happens to use the same shape underneath. If you ever publish the cowire trust-scope module in isolation, cite Tessera.
...
2. **`001_bom_schema.sql`** — adds the `boms`, `bom_steps`, `bom_versions`, `connectors`, `no_go_list`, `profile_config`, `profile_state` tables to `~/.cowire/cowire.db`. Idempotent. Add to `src/persistence.ts`'s init sequence after the existing tables. Boot default for `profile_state.active_mode` is `balanced`.
...
5. **`steward-planner.ts`** — the planning loop. Takes a goal + available capabilities + active profile, produces a BOM, persists it, emits `bom_proposed`. Has a `replan()` for failure recovery. Wire to existing `LlmProvider`. Add a `propose_plan` MCP tool that calls it. Feature-flag behind `cowire.experimental.planner=true`.
...
- A new event kind `bom_proposed` lands when a chat session asks cowire to plan something.
...
- The connector interface lets anyone add Wiser/Unifi/Roblox/Unity/whatever as orange bricks. cowire handles auth, exposes their capabilities to the planner, applies no-go list, audits every exec.
...
| File | Purpose |
|------|---------|
| `POSITIONING.md` | One-pager — what cowire IS and IS NOT |
...
5. **Risk class taxonomy completeness** — 8 classes feels right but might need a ninth for "system-config" (changes to cowire itself, profile mode switches, no-go list edits). Validate against a week of real decisions.
