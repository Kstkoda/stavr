...
describe('trust scope — github write tool names match scopes (underscore)', () => {
  ...
  it('github_create_pr in-scope auto-approves with no decision_request', async () => {
    h = await makeTrustHarness({
      ghPlan: (args) => {
        if (args[0] === 'pr' && args[1] === 'create') {
          return { stdout: 'https://github.com/Kstkoda/cowire/pull/77' };
        }
        return { stdout: '' };
      },
    });
    ...
    const scopeId = await proposeAndGrant(h, {
      title: 'cowire PR opens',
      description: 'allow PR creation in Kstkoda/cowire',
      allowed_actions: [
        { tool: 'github_create_pr', param_constraints: { repo: 'Kstkoda/cowire' } },
      ],
  ...
  it('github_merge_pr in-scope auto-approves with no decision_request', async () => {
    ...
    const scopeId = await proposeAndGrant(h, {
      title: 'cowire merges',
      description: 'allow PR merge in Kstkoda/cowire',
      allowed_actions: [
        { tool: 'github_merge_pr', param_constraints: { repo: 'Kstkoda/cowire' } },
      ],
      expires_after_actions: 3,
    });
  ...
  it('github_create_pr on a different repo (out of scope) still opens decision_request', async () => {
    ...
    await proposeAndGrant(h, {
      title: 'cowire PR opens',
      description: 'allow PR creation in Kstkoda/cowire only',
      allowed_actions: [
        { tool: 'github_create_pr', param_constraints: { repo: 'Kstkoda/cowire' } },
      ],
      expires_after_actions: 3,
